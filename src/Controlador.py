...
def checkHandsAboveHead(image, results, draw=False, display=False):
    '''
    This function checks the posture (Jumping, Crouching or Standing) of the person in an image.
    Args:
        image:   The input image with a prominent person whose the posture needs to be checked.
    ...
    leftHand_y = int(results.pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_WRIST].y * height   )

    rightHand_y = int(results.pose_landmarks.landmark[mp_pose.PoseLandmark.LEFT_WRIST].y * height)
  
    rightEye_y = int(results.pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_EYE].y * height)

    if (leftHand_y < rightEye_y and rightHand_y > rightEye_y ):
        posture = 'Left Hand Up'
    elif (rightHand_y < rightEye_y and leftHand_y > rightEye_y ):
        posture = 'Right Hand Up'
    elif (rightHand_y < rightEye_y and leftHand_y < rightEye_y ):
    ...
    if draw:
        ...
        # Write the posture of the person on the image. 
        cv2.putText(output_image, posture, (5, 50), cv2.FONT_HERSHEY_PLAIN, 2, (255, 255, 255), 3)
...
while camera_video.isOpened():
    ...
    if results.pose_landmarks:
        ...
        if game_started:
            ...
            if (horizontal_position=='Left' and x_pos_index!=0) :
                pydirectinput.keyDown('left')
                x_pos_index = 0  
            elif (horizontal_position=='Right' and x_pos_index!=2) :
                pydirectinput.keyDown('right')
                x_pos_index=2
            elif (horizontal_position=='Center' and x_pos_index==2):
                pydirectinput.keyUp('right')
                x_pos_index=1
            elif (horizontal_position=='Center' and x_pos_index==0) :
                pydirectinput.keyUp('left')
                x_pos_index=1

            frame, posture = checkHandsAboveHead(frame, results, draw=True)
    
            # Check if the person has jumped.
            if (posture == 'Left Hand Up' and s_hands_Location != 'Left Hand Up'):
                pydirectinput.press('a')
                s_hands_Location = 'Left Hand Up'
            elif posture == 'Right Hand Up' and s_hands_Location != 'Right Hand Up':

                pydirectinput.press('b')
                s_hands_Location = 'Right Hand Up'
            elif posture == 'Both Hands Up' and s_hands_Location != 'Both Hands Up':

                pydirectinput.press('c')
                s_hands_Location = 'Both Hands Up'
                 
            elif posture == 'Hands Down' and s_hands_Location != 'Hands Down':
                s_hands_Location = 'Hands Down'
        ...
        if checkHandsJoined(frame, results)[1] == 'Hands Joined':
            ...
            if counter == num_of_frames:
                ...
                if not(game_started):
                    ...
                    pydirectinput.click(x=1300, y=800, button='left')
